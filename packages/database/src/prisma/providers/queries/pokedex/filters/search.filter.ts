 ...
 *   pikachu     → name search (case-insensitive contains on formName)
 */
...
/**
 * Resolves dex numbers for a family search via PokemonModel.
 *
 * Strategy:
 *   1. Find pokemon whose speciesName contains the term → collect familyIds.
 *   2. Return all distinct pokedexNumbers sharing those familyIds.
 *
 * Uses speciesName (not formName) so searching "charizard" also returns
 * Mega Charizard X/Y, not just the base form.
 */
...
export async function searchFilter(
	search?: NonNullable<BasePokedexInput["filters"]>["search"],
